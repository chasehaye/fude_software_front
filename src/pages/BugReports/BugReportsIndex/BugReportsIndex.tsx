...
import { indexProjectsBugReports } from '../../../utils/list-api.ts';
  ...
  useEffect(() => {
    async function getLists() {
      ...
      try {
        ...
        const data = await indexProjectsBugReports(payload);
        if (data) {
          setLists(data);
        }
      } catch (err) {
  ...
  return (
    <div className="flex flex-col min-h-screen font-roboto uppercase">
      ...
      <div className="ml-14 mt-2">
        <Link
          to="/bug-report/project/create"
          className="hover:text-hoverc cursor-pointer"
        >
          [Create_Project_For_Bug_Reports]
        </Link>
      </div>

      <div className="p-10 w-full text-center">
        <h1 className="text-[2rem] text-white pb-2">
          Bug_Reports_for_Projects
        </h1>
