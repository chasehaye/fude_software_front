...
function ResetPassword() {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            setFormData({ ...formData, error: "Failed to reset password. Link may be expired." });
        }
    }
    ...
    return (
        <>
            <div className="border-b border-edge text-center h-14 flex justify-center sm:justify-between w-full flex-shrink-0">
                <Link to="/" className="hidden sm:flex px-10 sm:px-20 sm:border-r sm:border-edge cursor-pointer select-none hover:bg-hoverc/10 transition-all hover:text-hoverc items-center justify-center">Home</Link>
                <Link to="/forgot-password" className="px-10 sm:border-l sm:border-edge whitespace-nowrap select-none cursor-pointer hover:bg-hoverc/10 transition-all hover:text-hoverc flex items-center justify-center">Request New Link</Link>
            </div>
            ...
            <div className="flex flex-col items-center justify-center flex-grow px-4 text-center">
                {!isSubmitted ? (
                    <form autoComplete="off" onSubmit={handleSubmit} className="flex flex-col w-full max-w-80 text-left">
                        ...
                        <p className="text-sm mb-6 text-center">Enter your new password below</p>
                        
                        <FormField fieldName="New Password" type="password" name="newPassword" value={formData.newPassword} onChange={handleChange} isRequired={true} />
                        ...
                        <FormSubmitButton buttonText={"Change Password"} isInvalid={isInvalid || isNotMatch} />
                        <FormError errorText={formData.error} />
                    </form>
                ) : (
                    <div className="max-w-80">
                        <p className="text-sm mb-6 text-center">
                            Your password reset has been completed. Proceed to log back into your account below.
                        </p>
                        <Link to="/login" className="text-sm underline hover:text-hoverc cursor-pointer">
                            Return to Login
                        </Link>
                    </div>
                )}
